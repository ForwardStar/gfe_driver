...
*/
    void SortledtonDriver::bc(uint64_t max_iterations, const char* dump2file) {
      tm.register_thread(0);
      SnapshotTransaction tx = tm.getSnapshotTransaction(ds, false);
      auto num_vertices = tx.vertex_count();
      pvector<float> scores(num_vertices, 0);
      pvector<double> path_counts(num_vertices);
      Bitmap succ(num_vertices);
      std::vector<SlidingQueue<uint32_t>::iterator> depth_index;
      SlidingQueue<uint32_t> queue(num_vertices);
      uint32_t sp = 0;
      for (uint32_t iter=0; iter < max_iterations; iter++) {
          uint32_t source = sp++ % num_vertices;
          path_counts.fill(0);
          ...
          // PBFS
          pvector<uint32_t> depths(num_vertices, -1);
          std::vector<std::atomic<bool>> occurred(num_vertices);
          depths[source] = 0;
          ...
          #pragma omp parallel
          {
              ...
              while (!queue.empty()) {
                  ...
                  for (auto q_iter = queue.begin(); q_iter < queue.end(); q_iter++) {
                      uint32_t u = *q_iter;
                      std::vector<std::pair<uint64_t, double>> u_neighbors;
                      SORTLEDTON_ITERATE_WITH_PROPERTIES_NAMED(tx, u, v, w, end_iteration, {
                        u_neighbors.emplace_back(v, w);
                      });
          ...
          pvector<float> deltas(num_vertices, 0);
          for (int d=depth_index.size()-2; d >= 0; d--) {
              #pragma omp parallel for schedule(dynamic, 64)
              for (auto it = depth_index[d]; it < depth_index[d+1]; it++) {
                  uint32_t u = *it;
                  float delta_u = 0;
                  std::vector<std::pair<uint64_t, double>> g_out_neigh;
                  SORTLEDTON_ITERATE_WITH_PROPERTIES_NAMED(tx, u, v, w, end_iteration, {
                    g_out_neigh.emplace_back(v, w);
                  });
