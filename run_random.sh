...
if [ -n "$1" ]; then
    ...
    if [ -n "$2" ]; then
        ...
        ./build/gfe_driver -G datasets/com-friendster.ungraph.el -u -l $1 -w $2 > results/$1/random/com-friendster.ungraph-random-$1
        if [ "$1" = "radixgraph" ]; then
        ...
        ./build/gfe_driver -G datasets/com-friendster.ungraph.el -u -l $1 -w $(nproc) > results/$1/random/com-friendster.ungraph-random-$1
        if [ "$1" = "radixgraph" ]; then
            ./optimizer datasets/com-lj.ungraph.el 8 > settings
            echo "0" >> settings
        fi
