...
if [ -n "$1" ]; then
    ...
    if [ -n "$3" ]; then
        ...
        if [ -e "datasets/com-lj.ungraph.properties" ]; then
            if [ "$1" = "radixgraph" ]; then
        ...
        if [ -e "datasets/com-orkut.ungraph.properties" ]; then
            if [ "$1" = "radixgraph" ]; then
        ...
        if [ -e "datasets/twitter-2010.properties" ]; then
            if [ "$1" = "radixgraph" ]; then
        ...
        if [ -e "datasets/graph500-24.properties" ]; then
            ...
            ./build/gfe_driver -G datasets/graph500-24.properties -u -l $1 -w $(nproc) -r $(nproc) -R 3 --blacklist cdlp > results/$1/analytics/graph500-24-analytics-$1
        fi
        if [ -e "datasets/uniform-24.properties" ]; then
            if [ "$1" = "radixgraph" ]; then
                ./optimizer datasets/uniform-24.v 8 
            fi
            ./build/gfe_driver -G datasets/uniform-24.properties -u -l $1 -w $(nproc) -r $(nproc) -R 3 --blacklist cdlp > results/$1/analytics/uniform-24-analytics-$1
        fi
        if [ -e "datasets/dota-league.properties" ]; then
            if [ "$1" = "radixgraph" ]; then
                ./optimizer datasets/dota-league.v 8 
            fi
            ./build/gfe_driver -G datasets/dota-league.properties -u -l $1 -w $(nproc) -r $(nproc) -R 3 --blacklist cdlp > results/$1/analytics/dota-league-analytics-$1
        fi
        if [ -e "datasets/com-lj.ungraph.properties" ]; then
            if [ "$1" = "radixgraph" ]; then
                ./optimizer datasets/com-lj.ungraph.v 8 
            fi
            ./build/gfe_driver -G datasets/com-lj.ungraph.properties -u -l $1 -w $(nproc) -r $(nproc) -R 3 --blacklist cdlp > results/$1/analytics/com-lj-analytics-$1
        fi
        if [ -e "datasets/com-orkut.ungraph.properties" ]; then
            if [ "$1" = "radixgraph" ]; then
                ./optimizer datasets/com-orkut.ungraph.v 8 
            fi
            ./build/gfe_driver -G datasets/com-orkut.ungraph.properties -u -l $1 -w $(nproc) -r $(nproc) -R 3 --blacklist cdlp > results/$1/analytics/com-orkut-analytics-$1
        fi
        if [ -e "datasets/twitter-2010.properties" ]; then
            if [ "$1" = "radixgraph" ]; then
                ./optimizer datasets/twitter-2010.v 8 
            fi
            ./build/gfe_driver -G datasets/twitter-2010.properties -u -l $1 -w $(nproc) -r $(nproc) -R 3 --blacklist cdlp > results/$1/analytics/twitter-2010-analytics-$1
        fi
    fi
    if [ "$1" = "radixgraph" ]; then
